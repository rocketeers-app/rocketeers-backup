...
while read DATABASE; do \
  mysqldump --user=%MYSQL_USER% --password=%MYSQL_PASSWORD% --add-drop-table --extended-insert --single-transaction --skip-comments $DATABASE | \
  gzip -9 | \
  pv -L 1m -q | \
...
for DIR in /var/www/*; do \
  ...
  tar -cpf - --exclude="$DIR/persistent/storage/app/public/cache" $DIR/.env $DIR/certs $DIR/conf $DIR/persistent | \
  gzip -9 | \
  pv -L 1m -q | \
