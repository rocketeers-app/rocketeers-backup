...
# sites
for DIR in /var/www/!(default)/; do SITE=$(basename "$DIR"); tar -cf - $DIR | gzip -9 | s3cmd --acl-private put - s3://%S3_BUCKET%/rocketeers/backups/%SERVER%/$SITE/files/$DAY.tar.gz; done
